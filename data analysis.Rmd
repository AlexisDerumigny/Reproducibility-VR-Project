...
my_df <- my_df %>%
  mutate(Condition =
           factor(recode(Condition,
                         HED = 'Head Oriented', HIP = 'Hip Oriented',
                         STF = 'Standing Foot Velocity', AVG =  'Avg Feet Oriented' ),
                  levels = c("Head Oriented", "Hip Oriented",
                             "Standing Foot Velocity", "Avg Feet Oriented")
           ) )
```
...
for (i_question in 1:61){
  if (data_questions$ToBeInverted[i_question] == -1){
    my_df[, i_question + 3] = 6 - as.numeric(my_df[, i_question + 3])        #invert if 1 is best and 5 is worst, to 1 is worst and 5 is best
  } else if (data_questions$ToBeInverted[i_question] == -2){                 
    my_df[, i_question + 3] = 5 - as.numeric(my_df[, i_question + 3]) * 4/3  # this scaling is for the VR sickness questions since those are Likert scale 0 to 3, and should now be mapped from 1 to 5 to normalize
  } else {
    my_df[, i_question + 3] = as.numeric(my_df[, i_question + 3])            # else, keep default scaling.
  }
} 
```
...
my_df_pivot$value = (my_df_pivot$value - 1) * 100 / 4

# Renaming SSQ to VR Sickness
my_df_pivot$QuestionGroup = if_else(condition = my_df_pivot$QuestionGroup == "SSQ",
                                    true = "VR Sickness",
                                    false = my_df_pivot$QuestionGroup)
```
