...
#dataUX$orderCondition = sample.int(4, size = 80, replace = TRUE)
...
```{r}
my_model_emmeans_LocomotionTechnique$`pairwise differences of LocomotionTechnique` %>%
  as.data.frame() %>%
  mutate(`Name of difference` = `1`) %>%
  select(`Name of difference`, estimate, SE, p.value) %>%
  gt() %>%
  # tail(, -2) %>%
  tab_header(
...
result = pairs(my_model_emmeans, simple = "grp_Hip")
print(result)
```
